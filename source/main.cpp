...
int main_loop (void)
{
    ...
    glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, 256, 192, 0, GL_RGB, GL_FLOAT, NULL);
    ...
    glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, 192, 1, 0, GL_RGB, GL_FLOAT, NULL);
    ...
    while (state.run != RUN_STATE_EXIT)
    {
        ...
        /* Advance to the next frame generated by the console */
        Video_Frame *frame = snepulator_get_next_frame ();

        /* Scale the image to a multiple of SMS resolution */
        state.video_scale = ((float) state.host_width / (frame->width * state.video_par)) < ((float) state.host_height / frame->height) ?
                            ((float) state.host_width / (frame->width * state.video_par)) : ((float) state.host_height / frame->height);
...
int main (int argc, char **argv)
{
    ...
    window = SDL_CreateWindow ("Snepulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                               (256  + 16) * 3 * state.video_par, 224 * 3,
