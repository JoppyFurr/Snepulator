...
/*
 * Update flags for addx.b instructions.
 */
static inline void m68k_addx_b_flags (M68000_Context *context, uint8_t a, uint8_t b, uint8_t c, uint8_t result)
{
    context->state.ccr_negative = ((int8_t) result < 0);
    if (result != 0)
    {
    ...
    context->state.ccr_overflow = (int8_t) a + (int8_t) b + (int8_t) c > 127 || (int8_t) a + (int8_t) b + (int8_t) c < -128;
    context->state.ccr_carry = ((uint32_t) a + b + c) >> 8;
    context->state.ccr_extend = ((uint32_t) a + b + c) >> 8;
}


/*
 * Update flags for addx.w instructions.
 */
static inline void m68k_addx_w_flags (M68000_Context *context, uint16_t a, uint16_t b, uint16_t c, uint16_t result)
{
    context->state.ccr_negative = ((int16_t) result < 0);
    if (result != 0)
    {
    ...
    context->state.ccr_overflow = (int16_t) a + (int16_t) b + (int16_t) c > 32767 || (int16_t) a + (int16_t) b + (int16_t) c < -32768;
    context->state.ccr_carry = ((uint32_t) a + b + c) >> 16;
    context->state.ccr_extend = ((uint32_t) a + b + c) >> 16;
}


/*
 * Update flags for addx.l instructions.
 */
static inline void m68k_addx_l_flags (M68000_Context *context, uint32_t a, uint32_t b, uint32_t c, uint32_t result)
{
    context->state.ccr_negative = ((int32_t) result < 0);
    if (result != 0)
    {
    ...
    context->state.ccr_overflow = (int64_t)(int32_t) a + (int32_t) b + (int32_t) c > 2147483647 ||
                                  (int64_t)(int32_t) a + (int32_t) b + (int32_t) c < -2147483648;
    context->state.ccr_carry = ((uint64_t) a + b + c) >> 32;
    context->state.ccr_extend = ((uint64_t) a + b + c) >> 32;
}
...
/* ori.b (An) ← (An) | #xx */
static uint32_t m68k_0010_ori_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
...
/* ori.b (An+) ← (An+) | #xx */
static uint32_t m68k_0018_ori_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    uint32_t address = context->state.a [reg];
...
/* ori.b (-An) ← (-An) | #xx */
static uint32_t m68k_0020_ori_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* ori.b d(An+Xi) ← d(An+Xi) | #xx */
static uint32_t m68k_0030_ori_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
...
/* ori.b (xxx.l) ← (xxx.l) | #xx */
static uint32_t m68k_0039_ori_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t imm = read_extension (context);
    uint32_t address = read_extension_long (context);
...
/* ori.w (An) ← (An) | #xxxx */
static uint32_t m68k_0050_ori_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t imm = read_extension (context);
    uint32_t address = context->state.a [reg];
...
/* ori.w (An+) ← (An+) | #xxxx */
static uint32_t m68k_0058_ori_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t imm = read_extension (context);
    uint32_t address = context->state.a [reg];
...
/* ori.w (-An) ← (-An) | #xxxx */
static uint32_t m68k_0060_ori_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t imm = read_extension (context);
    context->state.a [reg] -= 2;
...
/* ori.w d(An) ← d(An) | #xxxx */
static uint32_t m68k_0068_ori_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t imm = read_extension (context);
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
...
/* ori.w d(An+Xi) ← d(An+Xi) | #xxxx */
static uint32_t m68k_0070_ori_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t imm = read_extension (context);
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* ori.w (xxx.w) ← (xxx.w) | #xxxx */
static uint32_t m68k_0078_ori_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t imm = read_extension (context);
    uint32_t address = (int16_t) read_extension (context);
...
/* ori.w (xxx.l) ← (xxx.l) | #xxxx */
static uint32_t m68k_0079_ori_w_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t imm = read_extension (context);
    uint32_t address = read_extension_long (context);
...
/* ori.l Dn ← Dn | #xxxxxxxx */
static uint32_t m68k_0080_ori_l_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* ori.l (An) ← (An) | #xxxxxxxx */
static uint32_t m68k_0090_ori_l_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t imm = read_extension_long (context);
    uint32_t address = context->state.a [reg];

    uint32_t result = read_long (context, address) | imm;
...
/* ori.l (An+) ← (An+) | #xxxxxxxx */
static uint32_t m68k_0098_ori_l_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t imm = read_extension_long (context);
    uint32_t address = context->state.a [reg];
...
/* ori.l (-An) ← (-An) | #xxxxxxxx */
static uint32_t m68k_00a0_ori_l_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t imm = read_extension_long (context);
    context->state.a [reg] -= 4;
...
/* ori.l d(An) ← d(An) | #xxxxxxxx */
static uint32_t m68k_00a8_ori_l_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t imm = read_extension_long (context);
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
...
/* ori.l d(An+Xi) ← d(An+Xi) | #xxxxxxxx */
static uint32_t m68k_00b0_ori_l_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t imm = read_extension_long (context);
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* ori.l (xxx.w) ← (xxx.w) | #xxxxxxxx */
static uint32_t m68k_00b8_ori_l_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint32_t address = (int16_t) read_extension (context);
...
/* ori.l (xxx.l) ← (xxx.l) | #xxxxxxxx */
static uint32_t m68k_00b9_ori_l_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint32_t address = read_extension_long (context);
...
/* bchg.l Dn [Dn] */
static uint32_t m68k_0140_bchg_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
...
static uint32_t m68k_0150_bchg_b_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
    uint32_t address = context->state.a [data_reg];
...
/* bchg.b (An+) [Dn] */
static uint32_t m68k_0158_bchg_b_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
    uint32_t address = context->state.a [data_reg];
...
/* bchg.b (-An) [Dn] */
static uint32_t m68k_0160_bchg_b_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
    context->state.a [data_reg] -= (data_reg == 7) ? 2 : 1;
...
/* bchg.b d(An) [Dn] */
static uint32_t m68k_0168_bchg_b_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
...
/* bchg.b d(An+Xi) [Dn] */
static uint32_t m68k_0170_bchg_b_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t data_reg = instruction & 0x07;
    uint16_t bit_reg = (instruction >> 9) & 0x07;
...
/* bchg.b (xxx.w) [Dn] */
static uint32_t m68k_0178_bchg_b_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t bit_reg = (instruction >> 9) & 0x07;
...
/* bchg.b (xxx.l) [Dn] */
static uint32_t m68k_0179_bchg_b_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t bit_reg = (instruction >> 9) & 0x07;
...
/* andi.b (An) ← (An) & #xx */
static uint32_t m68k_0210_andi_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    uint32_t address = context->state.a [reg];
...
/* andi.b (An+) ← (An+) & #xx */
static uint32_t m68k_0218_andi_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    uint32_t address = context->state.a [reg];
...
/* andi.b (-An) ← (-An) & #xx */
static uint32_t m68k_0220_andi_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* andi.b d(An+Xi) ← d(An+Xi) & #xx */
static uint32_t m68k_0230_andi_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t imm = read_extension (context);
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* andi.b (xxx.l) ← (xxx.l) & #xx */
static uint32_t m68k_0239_andi_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t imm = read_extension (context);
    uint32_t address = read_extension_long (context);
...
/* bchg.b (An) [#xx] */
static uint32_t m68k_0850_bchg_b_an_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t bit = read_extension (context) & 0x07;
    uint32_t address = context->state.a [reg];
...
/* bchg.b (An+) [#xx] */
static uint32_t m68k_0858_bchg_b_anp_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t bit = read_extension (context) & 0x07;
    uint32_t address = context->state.a [reg];
...
/* bchg.b (-An) [#xx] */
static uint32_t m68k_0860_bchg_b_pan_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t bit = read_extension (context) & 0x07;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* bchg.b d(An+Xi) [#xx] */
static uint32_t m68k_0870_bchg_b_danxi_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint16_t bit = read_extension (context) & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* bchg.b (xxx.w) [#xx] */
static uint32_t m68k_0878_bchg_b_aw_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t bit = read_extension (context) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* bchg.b (xxx.l) [#xx] */
static uint32_t m68k_0879_bchg_b_al_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t bit = read_extension (context) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* eori.w (An) ← (An) ^ #xxxx */
static uint32_t m68k_0a50_eori_w_an (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
...
/* eori.w (An+) ← (An+) ^ #xxxx */
static uint32_t m68k_0a58_eori_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
...
/* eori.w (-An) ← (-An) ^ #xxxx */
static uint32_t m68k_0a60_eori_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
...
/* eori.w d(An) ← d(An) ^ #xxxx */
static uint32_t m68k_0a68_eori_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
...
/* eori.w d(An+Xi) ← d(An+Xi) ^ #xxxx */
static uint32_t m68k_0a70_eori_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* eori.w (xxx.w) ← (xxx.w) ^ #xxxx */
static uint32_t m68k_0a78_eori_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint32_t address = (int16_t) read_extension (context);
...
/* eori.w (xxx.l) ← (xxx.l) ^ #xxxx */
static uint32_t m68k_0a79_eori_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension (context);
    uint32_t address = read_extension_long (context);
...
/* eori.l (An) ← (An) ^ #xxxxxxxx */
static uint32_t m68k_0a90_eori_l_an (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
...
/* eori.l (An+) ← (An+) ^ #xxxxxxxx */
static uint32_t m68k_0a98_eori_l_anp (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
...
/* eori.l (-An) ← (-An) ^ #xxxxxxxx */
static uint32_t m68k_0aa0_eori_l_pan (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 4;
...
/* eori.l d(An) ← d(An) ^ #xxxxxxxx */
static uint32_t m68k_0aa8_eori_l_dan (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
...
/* eori.l d(An+Xi) ← d(An+Xi) ^ #xxxxxxxx */
static uint32_t m68k_0ab0_eori_l_danxi (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* eori.l (xxx.w) ← (xxx.w) ^ #xxxxxxxx */
static uint32_t m68k_0ab8_eori_l_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint32_t address = (int16_t) read_extension (context);
...
/* eori.l (xxx.l) ← (xxx.l) ^ #xxxxxxxx */
static uint32_t m68k_0ab9_eori_l_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t imm = read_extension_long (context);
    uint32_t address = read_extension_long (context);
...
/* clr.b Dn */
static uint32_t m68k_4200_clr_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* clr.w Dn */
static uint32_t m68k_4240_clr_w_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;

    context->state.d [reg].w = 0x0000;
    m68k_clr_flags (context);
...
/* clr.w (An) */
static uint32_t m68k_4250_clr_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;

    write_word (context, context->state.a [reg], 0x0000);
    m68k_clr_flags (context);
...
/* clr.w (An+) */
static uint32_t m68k_4258_clr_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* clr.w (-An) */
static uint32_t m68k_4260_clr_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* clr.w d(An+Xi) */
static uint32_t m68k_4270_clr_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;

    write_word_with_index (context, context->state.a [reg], 0x00);
    m68k_clr_flags (context);
...
/* clr.w (xxx.l) */
static uint32_t m68k_4279_clr_w_al (M68000_Context *context, uint16_t instruction)
{
    write_word_al (context, 0x0000);
    m68k_clr_flags (context);
...
/* addq.w (-An), #xx */
static uint32_t m68k_5060_addq_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* addq.w d(An+Xi), #xx */
static uint32_t m68k_5070_addq_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* addq.w (xxx.l), #xx */
static uint32_t m68k_5079_addq_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
...
/* addq.l Dn, #xx */
static uint32_t m68k_5080_addq_l_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* addq.l (An), #xx */
static uint32_t m68k_5090_addq_l_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
...
/* addq.l (An+), #xx */
static uint32_t m68k_5098_addq_l_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 4;
...
/* addq.l (-An), #xx */
static uint32_t m68k_50a0_addq_l_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 4;
    uint32_t address = context->state.a [reg];
...
/* addq.l d(An), #xx */
static uint32_t m68k_50a8_addq_l_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
...
/* addq.l d(An+Xi), #xx */
static uint32_t m68k_50b0_addq_l_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* addq.l (xxx.l), #xx */
static uint32_t m68k_50b9_addq_l_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
...
/* subq.b (An+), #xx */
static uint32_t m68k_5118_subq_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += (reg == 7) ? 2 : 1;
...
/* subq.b (-An), #xx */
static uint32_t m68k_5120_subq_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
    uint32_t address = context->state.a [reg];
...
/* subq.b (xxx.l), #xx */
static uint32_t m68k_5139_subq_b_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
...
/* subq.w (-An), #xx */
static uint32_t m68k_5160_subq_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* subq.w d(An+Xi), #xx */
static uint32_t m68k_5170_subq_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
...
/* subq.w (xxx.l), #xx */
static uint32_t m68k_5179_subq_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
...
/* sf.b Dn */
static uint32_t m68k_51c0_sf_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b (An) */
static uint32_t m68k_51d0_sf_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b (An+) */
static uint32_t m68k_51d8_sf_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b (-An) */
static uint32_t m68k_51e0_sf_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b d(An) */
static uint32_t m68k_51e8_sf_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b d(An+Xi) */
static uint32_t m68k_51f0_sf_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
...
/* sf.b (xxx.w) */
static uint32_t m68k_51f8_sf_b_aw (M68000_Context *context, uint16_t instruction)
{
    write_byte_aw (context, 0x00);
...
/* sf.b (xxx.l) */
static uint32_t m68k_51f9_sf_b_al (M68000_Context *context, uint16_t instruction)
{
    write_byte_al (context, 0x00);
...
/* shi.b Dn */
static uint32_t m68k_52c0_shi_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
...
/* shi.b (An) */
static uint32_t m68k_52d0_shi_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* shi.b (An+) */
static uint32_t m68k_52d8_shi_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* shi.b (-An) */
static uint32_t m68k_52e0_shi_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* shi.b d(An) */
static uint32_t m68k_52e8_shi_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
...
/* shi.b d(An+Xi) */
static uint32_t m68k_52f0_shi_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
...
/* shi.b (xxx.w) */
static uint32_t m68k_52f8_shi_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* shi.b (xxx.l) */
static uint32_t m68k_52f9_shi_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_carry && !context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* sls.b Dn */
static uint32_t m68k_53c0_sls_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* sls.b (An) */
static uint32_t m68k_53d0_sls_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* sls.b (An+) */
static uint32_t m68k_53d8_sls_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* sls.b (-An) */
static uint32_t m68k_53e0_sls_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* sls.b d(An) */
static uint32_t m68k_53e8_sls_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* sls.b d(An+Xi) */
static uint32_t m68k_53f0_sls_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* sls.b (xxx.w) */
static uint32_t m68k_53f8_sls_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* sls.b (xxx.l) */
static uint32_t m68k_53f9_sls_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_carry || context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* scc.b Dn */
static uint32_t m68k_54c0_scc_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* scc.b (An) */
static uint32_t m68k_54d0_scc_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* scc.b (An+) */
static uint32_t m68k_54d8_scc_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* scc.b (-An) */
static uint32_t m68k_54e0_scc_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* scc.b d(An) */
static uint32_t m68k_54e8_scc_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* scc.b d(An+Xi) */
static uint32_t m68k_54f0_scc_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* scc.b (xxx.w) */
static uint32_t m68k_54f8_scc_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* scc.b (xxx.l) */
static uint32_t m68k_54f9_scc_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* scs.b Dn */
static uint32_t m68k_55c0_scs_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* scs.b (An) */
static uint32_t m68k_55d0_scs_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* scs.b (An+) */
static uint32_t m68k_55d8_scs_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* scs.b (-An) */
static uint32_t m68k_55e0_scs_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* scs.b d(An) */
static uint32_t m68k_55e8_scs_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* scs.b d(An+Xi) */
static uint32_t m68k_55f0_scs_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* scs.b (xxx.w) */
static uint32_t m68k_55f8_scs_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* scs.b (xxx.l) */
static uint32_t m68k_55f9_scs_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_carry) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* seq.b Dn */
static uint32_t m68k_57c0_seq_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* seq.b (An) */
static uint32_t m68k_57d0_seq_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* seq.b (An+) */
static uint32_t m68k_57d8_seq_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* seq.b (-An) */
static uint32_t m68k_57e0_seq_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* seq.b d(An) */
static uint32_t m68k_57e8_seq_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* seq.b d(An+Xi) */
static uint32_t m68k_57f0_seq_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* seq.b (xxx.w) */
static uint32_t m68k_57f8_seq_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* seq.b (xxx.l) */
static uint32_t m68k_57f9_seq_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_zero) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* svc.b Dn */
static uint32_t m68k_58c0_svc_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* svc.b (An) */
static uint32_t m68k_58d0_svc_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* svc.b (An+) */
static uint32_t m68k_58d8_svc_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* svc.b (-An) */
static uint32_t m68k_58e0_svc_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* svc.b d(An) */
static uint32_t m68k_58e8_svc_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* svc.b d(An+Xi) */
static uint32_t m68k_58f0_svc_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* svc.b (xxx.w) */
static uint32_t m68k_58f8_svc_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* svc.b (xxx.l) */
static uint32_t m68k_58f9_svc_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* svs.b Dn */
static uint32_t m68k_59c0_svs_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* svs.b (An) */
static uint32_t m68k_59d0_svs_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* svs.b (An+) */
static uint32_t m68k_59d8_svs_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* svs.b (-An) */
static uint32_t m68k_59e0_svs_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* svs.b d(An) */
static uint32_t m68k_59e8_svs_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* svs.b d(An+Xi) */
static uint32_t m68k_59f0_svs_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* svs.b (xxx.w) */
static uint32_t m68k_59f8_svs_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* svs.b (xxx.l) */
static uint32_t m68k_59f9_svs_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_overflow) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* spl.b (An) */
static uint32_t m68k_5ad0_spl_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* spl.b (An+) */
static uint32_t m68k_5ad8_spl_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* spl.b (-An) */
static uint32_t m68k_5ae0_spl_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* spl.b d(An) */
static uint32_t m68k_5ae8_spl_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* spl.b d(An+Xi) */
static uint32_t m68k_5af0_spl_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* spl.b (xxx.w) */
static uint32_t m68k_5af8_spl_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* spl.b (xxx.l) */
static uint32_t m68k_5af9_spl_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (!context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* smi.b Dn */
static uint32_t m68k_5bc0_smi_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* smi.b (An) */
static uint32_t m68k_5bd0_smi_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* smi.b (An+) */
static uint32_t m68k_5bd8_smi_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte (context, context->state.a [reg], value);
...
/* smi.b (-An) */
static uint32_t m68k_5be0_smi_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    context->state.a [reg] -= (reg == 7) ? 2 : 1;
...
/* smi.b d(An) */
static uint32_t m68k_5be8_smi_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_with_displacement (context, context->state.a [reg], value);
...
/* smi.b d(An+Xi) */
static uint32_t m68k_5bf0_smi_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_with_index (context, context->state.a [reg], value);
...
/* smi.b (xxx.w) */
static uint32_t m68k_5bf8_smi_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* smi.b (xxx.l) */
static uint32_t m68k_5bf9_smi_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = (context->state.ccr_negative) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* sge.b Dn */
static uint32_t m68k_5cc0_sge_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* sge.b (An) */
static uint32_t m68k_5cd0_sge_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sge.b (An+) */
static uint32_t m68k_5cd8_sge_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sge.b (-An) */
static uint32_t m68k_5ce0_sge_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sge.b d(An) */
static uint32_t m68k_5ce8_sge_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sge.b d(An+Xi) */
static uint32_t m68k_5cf0_sge_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sge.b (xxx.w) */
static uint32_t m68k_5cf8_sge_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* sge.b (xxx.l) */
static uint32_t m68k_5cf9_sge_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* slt.b Dn */
static uint32_t m68k_5dc0_slt_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b (An) */
static uint32_t m68k_5dd0_slt_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b (An+) */
static uint32_t m68k_5dd8_slt_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b (-An) */
static uint32_t m68k_5de0_slt_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b d(An) */
static uint32_t m68k_5de8_slt_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b d(An+Xi) */
static uint32_t m68k_5df0_slt_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* slt.b (xxx.w) */
static uint32_t m68k_5df8_slt_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* slt.b (xxx.l) */
static uint32_t m68k_5df9_slt_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* sgt.b Dn */
static uint32_t m68k_5ec0_sgt_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* sgt.b (An) */
static uint32_t m68k_5ed0_sgt_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
...
/* sgt.b (An+) */
static uint32_t m68k_5ed8_sgt_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
...
/* sgt.b (-An) */
static uint32_t m68k_5ee0_sgt_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
...
/* sgt.b d(An) */
static uint32_t m68k_5ee8_sgt_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
...
/* sgt.b d(An+Xi) */
static uint32_t m68k_5ef0_sgt_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
...
/* sgt.b (xxx.w) */
static uint32_t m68k_5ef8_sgt_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* sgt.b (xxx.l) */
static uint32_t m68k_5ef9_sgt_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_negative && context->state.ccr_overflow && !context->state.ccr_zero) ||
                     (!context->state.ccr_negative && !context->state.ccr_overflow && !context->state.ccr_zero)) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* sle.b Dn */
static uint32_t m68k_5fc0_sle_b_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
    context->state.d [reg].b = value;
...
/* sle.b (An) */
static uint32_t m68k_5fd0_sle_b_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sle.b (An+) */
static uint32_t m68k_5fd8_sle_b_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sle.b (-An) */
static uint32_t m68k_5fe0_sle_b_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sle.b d(An) */
static uint32_t m68k_5fe8_sle_b_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sle.b d(An+Xi) */
static uint32_t m68k_5ff0_sle_b_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
...
/* sle.b (xxx.w) */
static uint32_t m68k_5ff8_sle_b_aw (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_aw (context, value);
...
/* sle.b (xxx.l) */
static uint32_t m68k_5ff9_sle_b_al (M68000_Context *context, uint16_t instruction)
{
    uint8_t value = ((context->state.ccr_zero) || (context->state.ccr_negative && !context->state.ccr_overflow) ||
                     (!context->state.ccr_negative && context->state.ccr_overflow)) ? 0xff : 0x00;
    write_byte_al (context, value);
...
/* or.b Dn ← Dn | (An) */
static uint32_t m68k_8010_or_b_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | (An+) */
static uint32_t m68k_8018_or_b_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | (-An) */
static uint32_t m68k_8020_or_b_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | d(An+Xi) */
static uint32_t m68k_8030_or_b_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | (xxx.w) */
static uint32_t m68k_8038_or_b_dn_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | (xxx.l) */
static uint32_t m68k_8039_or_b_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | d(PC) */
static uint32_t m68k_803a_or_b_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | d(PC+Xi) */
static uint32_t m68k_803b_or_b_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b Dn ← Dn | #xx */
static uint32_t m68k_803c_or_b_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | (An) */
static uint32_t m68k_8050_or_w_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | (An+) */
static uint32_t m68k_8058_or_w_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | (-An) */
static uint32_t m68k_8060_or_w_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | d(An) */
static uint32_t m68k_8068_or_w_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | d(An+Xi) */
static uint32_t m68k_8070_or_w_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | (xxx.w) */
static uint32_t m68k_8078_or_w_dn_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | (xxx.l) */
static uint32_t m68k_8079_or_w_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | d(PC) */
static uint32_t m68k_807a_or_w_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | d(PC+Xi) */
static uint32_t m68k_807b_or_w_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.w Dn ← Dn | #xxxx */
static uint32_t m68k_807c_or_w_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | Dn */
static uint32_t m68k_8080_or_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | (An) */
static uint32_t m68k_8090_or_l_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | (An+) */
static uint32_t m68k_8098_or_l_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | (-An) */
static uint32_t m68k_80a0_or_l_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | d(An) */
static uint32_t m68k_80a8_or_l_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | d(An+Xi) */
static uint32_t m68k_80b0_or_l_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | (xxx.w) */
static uint32_t m68k_80b8_or_l_dn_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | (xxx.l) */
static uint32_t m68k_80b9_or_l_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | d(PC) */
static uint32_t m68k_80ba_or_l_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | d(PC+Xi) */
static uint32_t m68k_80bb_or_l_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.l Dn ← Dn | #xxxxxxxx */
static uint32_t m68k_80bc_or_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* or.b (An) ← (An) | Dn */
static uint32_t m68k_8110_or_b_an_dn (M68000_Context *context, uint16_t instruction)
{
...
/* or.b (An+) ← (An+) | Dn */
static uint32_t m68k_8118_or_b_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* or.b (-An) ← (-An) | Dn */
static uint32_t m68k_8120_or_b_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= (dest_reg == 7) ? 2 : 1;
...
/* or.b d(An+Xi) ← d(An+Xi) | Dn */
static uint32_t m68k_8130_or_b_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* or.b (xxx.w) ← (xxx.w) | Dn */
static uint32_t m68k_8138_or_b_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* or.b (xxx.l) ← (xxx.l) | Dn */
static uint32_t m68k_8139_or_b_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* or.w (An) ← (An) & Dn */
static uint32_t m68k_8150_or_w_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* or.w (An+) ← (An+) & Dn */
static uint32_t m68k_8158_or_w_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* or.w (-An) ← (-An) & Dn */
static uint32_t m68k_8160_or_w_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 2;
...
/* or.w d(An) ← d(An) & Dn */
static uint32_t m68k_8168_or_w_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* or.w d(An+Xi) ← d(An+Xi) & Dn */
static uint32_t m68k_8170_or_w_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* or.w (xxx.w) ← (xxx.w) & Dn */
static uint32_t m68k_8178_or_w_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* or.w (xxx.l) ← (xxx.l) & Dn */
static uint32_t m68k_8179_or_w_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* or.l (An) ← (An) & Dn */
static uint32_t m68k_8190_or_l_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* or.l (An+) ← (An+) & Dn */
static uint32_t m68k_8198_or_l_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* or.l (-An) ← (-An) & Dn */
static uint32_t m68k_81a0_or_l_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 4;
...
/* or.l d(An) ← d(An) & Dn */
static uint32_t m68k_81a8_or_l_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* or.l d(An+Xi) ← d(An+Xi) & Dn */
static uint32_t m68k_81b0_or_l_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* or.l (xxx.w) ← (xxx.w) & Dn */
static uint32_t m68k_81b8_or_l_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* or.l (xxx.l) ← (xxx.l) & Dn */
static uint32_t m68k_81b9_or_l_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* cmp.w Dn - (An+) */
static uint32_t m68k_b058_cmp_w_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - (-An) */
static uint32_t m68k_b060_cmp_w_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - d(An+Xi) */
static uint32_t m68k_b070_cmp_w_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - (xxx.l) */
static uint32_t m68k_b079_cmp_w_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - d(PC) */
static uint32_t m68k_b07a_cmp_w_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - d(PC+Xi) */
static uint32_t m68k_b07b_cmp_w_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.w Dn - #xxxx */
static uint32_t m68k_b07c_cmp_w_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - Dn */
static uint32_t m68k_b080_cmp_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - (An+) */
static uint32_t m68k_b098_cmp_l_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - (-An) */
static uint32_t m68k_b0a0_cmp_l_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - d(An) */
static uint32_t m68k_b0a8_cmp_l_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - d(An+Xi) */
static uint32_t m68k_b0b0_cmp_l_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - (xxx.l) */
static uint32_t m68k_b0b9_cmp_l_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - d(PC) */
static uint32_t m68k_b0ba_cmp_l_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - d(PC+Xi) */
static uint32_t m68k_b0bb_cmp_l_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* cmp.l Dn - #xxxxxxxx */
static uint32_t m68k_b0bc_cmp_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* eor.b (An) ← (An) ^ Dn */
static uint32_t m68k_b110_eor_b_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.b (An+) ← (An+) ^ Dn */
static uint32_t m68k_b118_eor_b_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.b (-An) ← (-An) ^ Dn */
static uint32_t m68k_b120_eor_b_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= (dest_reg == 7) ? 2 : 1;
...
/* eor.b d(An) ← d(An) ^ Dn */
static uint32_t m68k_b128_eor_b_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* eor.b d(An+Xi) ← d(An+Xi) ^ Dn */
static uint32_t m68k_b130_eor_b_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* eor.b (xxx.w) ← (xxx.w) ^ Dn */
static uint32_t m68k_b138_eor_b_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* eor.b (xxx.l) ← (xxx.l) ^ Dn */
static uint32_t m68k_b139_eor_b_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* eor.w (An) ← (An) ^ Dn */
static uint32_t m68k_b150_eor_w_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.w (An+) ← (An+) ^ Dn */
static uint32_t m68k_b158_eor_w_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.w (-An) ← (-An) ^ Dn */
static uint32_t m68k_b160_eor_w_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 2;
...
/* eor.w d(An) ← d(An) ^ Dn */
static uint32_t m68k_b168_eor_w_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* eor.w d(An+Xi) ← d(An+Xi) ^ Dn */
static uint32_t m68k_b170_eor_w_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* eor.w (xxx.w) ← (xxx.w) ^ Dn */
static uint32_t m68k_b178_eor_w_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* eor.w (xxx.l) ← (xxx.l) ^ Dn */
static uint32_t m68k_b179_eor_w_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* eor.l (An) ← (An) ^ Dn */
static uint32_t m68k_b190_eor_l_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.l (An+) ← (An+) ^ Dn */
static uint32_t m68k_b198_eor_l_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* eor.l (-An) ← (-An) ^ Dn */
static uint32_t m68k_b1a0_eor_l_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 4;
...
/* eor.l d(An) ← d(An) ^ Dn */
static uint32_t m68k_b1a8_eor_l_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* eor.l d(An+Xi) ← d(An+Xi) ^ Dn */
static uint32_t m68k_b1b0_eor_l_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* eor.l (xxx.w) ← (xxx.w) ^ Dn */
static uint32_t m68k_b1b8_eor_l_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* eor.l (xxx.l) ← (xxx.l) ^ Dn */
static uint32_t m68k_b1b9_eor_l_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* and.w Dn ← Dn & (An) */
static uint32_t m68k_c050_and_w_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & (An+) */
static uint32_t m68k_c058_and_w_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & (-An) */
static uint32_t m68k_c060_and_w_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & d(An) */
static uint32_t m68k_c068_and_w_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & d(An+Xi) */
static uint32_t m68k_c070_and_w_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & (xxx.w) */
static uint32_t m68k_c078_and_w_dn_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & (xxx.l) */
static uint32_t m68k_c079_and_w_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & d(PC) */
static uint32_t m68k_c07a_and_w_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w Dn ← Dn & #xxxx */
static uint32_t m68k_c07c_and_w_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & (An) */
static uint32_t m68k_c090_and_l_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & (An+) */
static uint32_t m68k_c098_and_l_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & (-An) */
static uint32_t m68k_c0a0_and_l_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & d(An) */
static uint32_t m68k_c0a8_and_l_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & d(An+Xi) */
static uint32_t m68k_c0b0_and_l_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & (xxx.w) */
static uint32_t m68k_c0b8_and_l_dn_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & (xxx.l) */
static uint32_t m68k_c0b9_and_l_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & d(PC) */
static uint32_t m68k_c0ba_and_l_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & d(PC+Xi) */
static uint32_t m68k_c0bb_and_l_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.l Dn ← Dn & #xxxx */
static uint32_t m68k_c0bc_and_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* and.w (An+) ← (An+) & Dn */
static uint32_t m68k_c158_and_w_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* and.w (-An) ← (-An) & Dn */
static uint32_t m68k_c160_and_w_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 2;
...
/* and.w d(An) ← d(An) & Dn */
static uint32_t m68k_c168_and_w_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* and.w d(An+Xi) ← d(An+Xi) & Dn */
static uint32_t m68k_c170_and_w_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* and.w (xxx.w) ← (xxx.w) & Dn */
static uint32_t m68k_c178_and_w_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* and.w (xxx.l) ← (xxx.l) & Dn */
static uint32_t m68k_c179_and_w_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* and.l (An) ← (An) & Dn */
static uint32_t m68k_c190_and_l_an_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* and.l (An+) ← (An+) & Dn */
static uint32_t m68k_c198_and_l_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* and.l (-An) ← (-An) & Dn */
static uint32_t m68k_c1a0_and_l_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 4;
...
/* and.l d(An) ← d(An) & Dn */
static uint32_t m68k_c1a8_and_l_dan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [dest_reg]);
...
/* and.l d(An+Xi) ← d(An+Xi) & Dn */
static uint32_t m68k_c1b0_and_l_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* and.l (xxx.w) ← (xxx.w) & Dn */
static uint32_t m68k_c1b8_and_l_aw_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = (int16_t) read_extension (context);
...
/* and.l (xxx.l) ← (xxx.l) & Dn */
static uint32_t m68k_c1b9_and_l_al_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint32_t address = read_extension_long (context);
...
/* add.l Dn ← Dn + (An) */
static uint32_t m68k_d090_add_l_dn_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + (An+) */
static uint32_t m68k_d098_add_l_dn_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + (-An) */
static uint32_t m68k_d0a0_add_l_dn_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + d(An) */
static uint32_t m68k_d0a8_add_l_dn_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + d(An+Xi) */
static uint32_t m68k_d0b0_add_l_dn_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + (xxx.l) */
static uint32_t m68k_d0b9_add_l_dn_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + d(PC) */
static uint32_t m68k_d0ba_add_l_dn_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + d(PC+Xi) */
static uint32_t m68k_d0bb_add_l_dn_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l Dn ← Dn + #xxxxxxxx */
static uint32_t m68k_d0bc_add_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.w An ← An + (An+) */
static uint32_t m68k_d0d8_adda_w_an_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.w An ← An + (-An) */
static uint32_t m68k_d0e0_adda_w_an_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.w An ← An + d(An) */
static uint32_t m68k_d0e8_adda_w_an_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.w An ← An + (xxx.w) */
static uint32_t m68k_d0f8_adda_w_an_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += (int16_t) read_word_aw (context);
...
/* adda.w An ← An + (xxx.l) */
static uint32_t m68k_d0f9_adda_w_an_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += (int16_t) read_word_al (context);
...
/* adda.w An ← An + d(PC) */
static uint32_t m68k_d0fa_adda_w_an_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += (int16_t) read_word_with_displacement (context, context->state.pc);

    return 0;
}


/* adda.w An ← An + d(PC+Xi) */
static uint32_t m68k_d0fb_adda_w_an_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* addx.b Dn ← Dn + Dn */
static uint32_t m68k_d100_addx_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    uint8_t c = context->state.ccr_extend;
    uint8_t b = context->state.d [source_reg].b;
    uint8_t a = context->state.d [dest_reg].b;
    uint8_t result = a + b + c;

    context->state.d [dest_reg].b = result;
    m68k_addx_b_flags (context, a, b, c, result);
...
/* addx.b (-An) ← (-An) + (-An) */
static uint32_t m68k_d108_addx_b_pan_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* addx.w Dn ← Dn + Dn */
static uint32_t m68k_d140_addx_w_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    uint16_t c = context->state.ccr_extend;
    uint16_t b = context->state.d [source_reg].w;
    uint16_t a = context->state.d [dest_reg].w;
    uint16_t result = a + b + c;

    context->state.d [dest_reg].w = result;
    m68k_addx_w_flags (context, a, b, c, result);
...
/* addx.w (-An) ← (-An) + (-An) */
static uint32_t m68k_d148_addx_w_pan_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* addx.l Dn ← Dn + Dn */
static uint32_t m68k_d180_addx_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    uint32_t c = context->state.ccr_extend;
    uint32_t b = context->state.d [source_reg].l;
    uint32_t a = context->state.d [dest_reg].l;
    uint32_t result = a + b + c;

    context->state.d [dest_reg].l = result;
    m68k_addx_l_flags (context, a, b, c, result);
...
/* addx.l (-An) ← (-An) + (-An) */
static uint32_t m68k_d188_addx_l_pan_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* add.l d(An+) ← (An+) + Dn */
static uint32_t m68k_d198_add_l_anp_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = context->state.a [dest_reg];
...
/* add.l d(-An) ← (-An) + Dn */
static uint32_t m68k_d1a0_add_l_pan_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    context->state.a [dest_reg] -= 4;
...
/* add.l d(An+Xi) ← d(An+Xi) + Dn */
static uint32_t m68k_d1b0_add_l_danxi_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = (instruction >> 9) & 0x07;
    uint16_t dest_reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [dest_reg]);
...
/* adda.l An ← An + An */
static uint32_t m68k_d1c8_adda_l_an_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + (An) */
static uint32_t m68k_d1d0_adda_l_an_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + (An+) */
static uint32_t m68k_d1d8_adda_l_an_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + (-An) */
static uint32_t m68k_d1e0_adda_l_an_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + d(An) */
static uint32_t m68k_d1e8_adda_l_an_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + d(An+Xi) */
static uint32_t m68k_d1f0_adda_l_an_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t source_reg = instruction & 0x07;
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + (xxx.w) */
static uint32_t m68k_d1f8_adda_l_an_aw (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += read_long_aw (context);
...
/* adda.l An ← An + (xxx.l) */
static uint32_t m68k_d1f9_adda_l_an_al (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += read_long_al (context);
...
/* adda.l An ← An + d(PC) */
static uint32_t m68k_d1fa_adda_l_an_dpc (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;

    context->state.a [dest_reg] += read_long_with_displacement (context, context->state.pc);

    return 0;
}


/* adda.l An ← An + d(PC+Xi) */
static uint32_t m68k_d1fb_adda_l_an_dpcxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
...
/* adda.l An ← An + #xxxx */
static uint32_t m68k_d1fc_adda_l_an_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t dest_reg = (instruction >> 9) & 0x07;
    uint32_t imm = read_extension_long (context);
...
/* asr.b Dn ← Dn >> #xx */
static uint32_t m68k_e000_asr_b_dn_imm (M68000_Context *context, uint16_t instruction)
{
...
/* roxr.b Dn ← Dn >> #xx */
static uint32_t m68k_e010_roxr_b_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint8_t value = context->state.d [reg].b;
    ...
    {
        context->state.ccr_carry = value & 0x01;
        value = (value >> 1) | (context->state.ccr_extend << 7);
...
/* asr.b Dn ← Dn >> Dn */
static uint32_t m68k_e020_asr_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
...
/* lsr.b Dn ← Dn >> Dn */
static uint32_t m68k_e028_lsr_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* roxr.b Dn ← Dn >> Dn */
static uint32_t m68k_e030_roxr_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint8_t value = context->state.d [reg].b;
    ...
    context->state.ccr_carry = context->state.ccr_extend;
    for (uint32_t i = 0; i < count; i++)
    {
        context->state.ccr_carry = value & 0x01;
        value = (value >> 1) | (context->state.ccr_extend << 7);
...
/* roxr.w Dn ← Dn >> #xx */
static uint32_t m68k_e050_roxr_w_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint16_t value = context->state.d [reg].w;
    ...
    {
        context->state.ccr_carry = value & 0x0001;
        value = (value >> 1) | (context->state.ccr_extend << 15);
        context->state.ccr_extend = context->state.ccr_carry;
    }
...
/* asr.w Dn ← Dn >> #xx */
static uint32_t m68k_e060_asr_w_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint16_t value = context->state.d [reg].w;
...
/* roxr.w Dn ← Dn >> Dn */
static uint32_t m68k_e070_roxr_w_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint16_t value = context->state.d [reg].w;
...
/* lsr.l Dn ← Dn >> #xx */
static uint32_t m68k_e088_lsr_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* roxr.l Dn ← Dn >> #xx */
static uint32_t m68k_e090_roxr_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
    ...
    {
        context->state.ccr_carry = value & 0x00000001;
        value = (value >> 1) | (context->state.ccr_extend << 31);
        context->state.ccr_extend = context->state.ccr_carry;
    }
...
/* asr.l Dn ← Dn >> Dn */
static uint32_t m68k_e0a0_asr_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* lsr.l Dn ← Dn >> Dn */
static uint32_t m68k_e0a8_lsr_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* roxr.l Dn ← Dn >> Dn */
static uint32_t m68k_e0b0_roxr_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* asr.w (An) ← (An) >> 1 */
static uint32_t m68k_e0d0_asr_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    uint16_t value = read_word (context, address);
...
/* asr.w (An+) ← (An+) >> 1 */
static uint32_t m68k_e0d8_asr_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 2;
...
/* asr.w (-An) ← (-An) >> 1 */
static uint32_t m68k_e0e0_asr_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* asr.w d(An+Xi) ← d(An+Xi) >> 1 */
static uint32_t m68k_e0f0_asr_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* asr.w (xxx.w) ← (xxx.w) >> 1 */
static uint32_t m68k_e0f8_asr_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = (int16_t) read_extension (context);
    uint16_t value = read_word (context, address);
...
/* asr.w (xxx.l) ← (xxx.l) >> 1 */
static uint32_t m68k_e0f9_asr_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
    uint16_t value = read_word (context, address);
...
/* asl.b Dn ← Dn << #xx */
static uint32_t m68k_e100_asl_b_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint8_t value = context->state.d [reg].b;

    uint8_t initial_value = value;
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        value = value << 1;
        sign_changed |= (value ^ initial_value) >> 7;
    }
...
/* roxl.b Dn ← Dn << #xx */
static uint32_t m68k_e110_roxl_b_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint8_t value = context->state.d [reg].b;
...
/* asl.b Dn ← Dn << Dn */
static uint32_t m68k_e120_asl_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint8_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* lsl.b Dn ← Dn << Dn */
static uint32_t m68k_e128_lsl_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint8_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* roxl.b Dn ← Dn << Dn */
static uint32_t m68k_e130_roxl_b_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint8_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* roxl.w Dn ← Dn << Dn */
static uint32_t m68k_e170_roxl_w_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint8_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* roxl.l Dn ← Dn << #xx */
static uint32_t m68k_e190_roxl_l_dn_imm (M68000_Context *context, uint16_t instruction)
{
    uint16_t count = (instruction & 0x0e00) ? ((instruction >> 9) & 0x07) : 8;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* asl.l Dn ← Dn << Dn */
static uint32_t m68k_e1a0_asl_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* lsl.l Dn ← Dn << Dn */
static uint32_t m68k_e1a8_lsl_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint16_t reg = instruction & 0x07;
    uint32_t value = context->state.d [reg].l;
...
/* roxl.l Dn ← Dn << Dn */
static uint32_t m68k_e1b0_roxl_l_dn_dn (M68000_Context *context, uint16_t instruction)
{
    uint16_t count_reg = (instruction >> 9) & 0x07;
    uint8_t count = context->state.d [count_reg].b & 0x3f;
    uint16_t reg = instruction & 0x07;
...
/* lsr.w (An) ← (An) >> 1 */
static uint32_t m68k_e2d0_lsr_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    uint16_t value = read_word (context, address);
...
/* lsr.w (An+) ← (An+) >> 1 */
static uint32_t m68k_e2d8_lsr_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 2;
...
/* lsr.w (-An) ← (-An) >> 1 */
static uint32_t m68k_e2e0_lsr_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* lsr.w d(An) ← d(An) >> 1 */
static uint32_t m68k_e2e8_lsr_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* lsr.w d(An+Xi) ← d(An+Xi) >> 1 */
static uint32_t m68k_e2f0_lsr_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* lsr.w (xxx.w) ← (xxx.w) >> 1 */
static uint32_t m68k_e2f8_lsr_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = (int16_t) read_extension (context);
    uint16_t value = read_word (context, address);
...
/* lsr.w (xxx.l) ← (xxx.l) >> 1 */
static uint32_t m68k_e2f9_lsr_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
    uint16_t value = read_word (context, address);
...
/* lsl.w (An) ← (An) << 1 */
static uint32_t m68k_e3d0_lsl_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    uint16_t value = read_word (context, address);
...
/* lsl.w (An+) ← (An+) << 1 */
static uint32_t m68k_e3d8_lsl_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 2;
...
/* lsl.w (-An) ← (-An) << 1 */
static uint32_t m68k_e3e0_lsl_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* lsl.w d(An) ← d(An) << 1 */
static uint32_t m68k_e3e8_lsl_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* lsl.w d(An+Xi) ← d(An+Xi) << 1 */
static uint32_t m68k_e3f0_lsl_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* lsl.w (xxx.w) ← (xxx.w) << 1 */
static uint32_t m68k_e3f8_lsl_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = (int16_t) read_extension (context);
    uint16_t value = read_word (context, address);
...
/* lsl.w (xxx.l) ← (xxx.l) << 1 */
static uint32_t m68k_e3f9_lsl_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
    uint16_t value = read_word (context, address);
...
/* roxr.w (An) ← (An) << 1 */
static uint32_t m68k_e4d0_roxr_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    uint16_t value = read_word (context, address);
...
/* roxr.w (An+) ← (An+) << 1 */
static uint32_t m68k_e4d8_roxr_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 2;
...
/* roxr.w (-An) ← (-An) << 1 */
static uint32_t m68k_e4e0_roxr_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* roxr.w d(An) ← d(An) << 1 */
static uint32_t m68k_e4e8_roxr_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* roxr.w d(An+Xi) ← d(An+Xi) << 1 */
static uint32_t m68k_e4f0_roxr_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* roxr.w d(xxx.w) ← d(xxx.w) << 1 */
static uint32_t m68k_e4f8_roxr_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = (int16_t) read_extension (context);
    uint16_t value = read_word (context, address);
...
/* roxr.w d(xxx.l) ← d(xxx.l) << 1 */
static uint32_t m68k_e4f9_roxr_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
    uint16_t value = read_word (context, address);
...
/* roxl.w (An) ← (An) << 1 */
static uint32_t m68k_e5d0_roxl_w_an (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    uint16_t value = read_word (context, address);
...
/* roxl.w (An+) ← (An+) << 1 */
static uint32_t m68k_e5d8_roxl_w_anp (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = context->state.a [reg];
    context->state.a [reg] += 2;
...
/* roxl.w (-An) ← (-An) << 1 */
static uint32_t m68k_e5e0_roxl_w_pan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    context->state.a [reg] -= 2;
    uint32_t address = context->state.a [reg];
...
/* roxl.w d(An) ← d(An) << 1 */
static uint32_t m68k_e5e8_roxl_w_dan (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_displacement (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* roxl.w d(An+Xi) ← d(An+Xi) << 1 */
static uint32_t m68k_e5f0_roxl_w_danxi (M68000_Context *context, uint16_t instruction)
{
    uint16_t reg = instruction & 0x07;
    uint32_t address = address_with_index (context, context->state.a [reg]);
    uint16_t value = read_word (context, address);
...
/* roxl.w d(xxx.w) ← d(xxx.w) << 1 */
static uint32_t m68k_e5f8_roxl_w_aw (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = (int16_t) read_extension (context);
    uint16_t value = read_word (context, address);
...
/* roxl.w d(xxx.l) ← d(xxx.l) << 1 */
static uint32_t m68k_e5f9_roxl_w_al (M68000_Context *context, uint16_t instruction)
{
    uint32_t address = read_extension_long (context);
    uint16_t value = read_word (context, address);
...
static void m68k_init_instructions (void)
{
    ...
    for (uint16_t data_reg = 0; data_reg < 8; data_reg++)
    {
        for (uint16_t bit_reg = 0; bit_reg < 8; bit_reg++)
        {
            ...
            m68k_instruction [0x0140 | (bit_reg << 9) | data_reg] = m68k_0140_bchg_l_dn_dn;
            m68k_instruction [0x0150 | (bit_reg << 9) | data_reg] = m68k_0150_bchg_b_an_dn;
            m68k_instruction [0x0158 | (bit_reg << 9) | data_reg] = m68k_0158_bchg_b_anp_dn;
            m68k_instruction [0x0160 | (bit_reg << 9) | data_reg] = m68k_0160_bchg_b_pan_dn;
            m68k_instruction [0x0168 | (bit_reg << 9) | data_reg] = m68k_0168_bchg_b_dan_dn;
            m68k_instruction [0x0170 | (bit_reg << 9) | data_reg] = m68k_0170_bchg_b_danxi_dn;
        ...
        m68k_instruction [0x0178 | (data_reg << 9)] = m68k_0178_bchg_b_aw_dn;
        m68k_instruction [0x0179 | (data_reg << 9)] = m68k_0179_bchg_b_al_dn;
        ...
        m68k_instruction [0x0850 | data_reg] = m68k_0850_bchg_b_an_imm;
        m68k_instruction [0x0858 | data_reg] = m68k_0858_bchg_b_anp_imm;
        m68k_instruction [0x0860 | data_reg] = m68k_0860_bchg_b_pan_imm;
        m68k_instruction [0x0868 | data_reg] = m68k_0868_bchg_b_dan_imm;
        m68k_instruction [0x0870 | data_reg] = m68k_0870_bchg_b_danxi_imm;
    ...
    m68k_instruction [0x0878] = m68k_0878_bchg_b_aw_imm;
    m68k_instruction [0x0879] = m68k_0879_bchg_b_al_imm;
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        ...
        m68k_instruction [0x0010 | reg] = m68k_0010_ori_b_an;
        m68k_instruction [0x0018 | reg] = m68k_0018_ori_b_anp;
        m68k_instruction [0x0020 | reg] = m68k_0020_ori_b_pan;
        m68k_instruction [0x0028 | reg] = m68k_0028_ori_b_dan;
        m68k_instruction [0x0030 | reg] = m68k_0030_ori_b_danxi;
        m68k_instruction [0x0040 | reg] = m68k_0040_ori_w_dn;
        m68k_instruction [0x0050 | reg] = m68k_0050_ori_w_an;
        m68k_instruction [0x0058 | reg] = m68k_0058_ori_w_anp;
        m68k_instruction [0x0060 | reg] = m68k_0060_ori_w_pan;
        m68k_instruction [0x0068 | reg] = m68k_0068_ori_w_dan;
        m68k_instruction [0x0070 | reg] = m68k_0070_ori_w_danxi;
        m68k_instruction [0x0080 | reg] = m68k_0080_ori_l_dn;
        m68k_instruction [0x0090 | reg] = m68k_0090_ori_l_an;
        m68k_instruction [0x0098 | reg] = m68k_0098_ori_l_anp;
        m68k_instruction [0x00a0 | reg] = m68k_00a0_ori_l_pan;
        m68k_instruction [0x00a8 | reg] = m68k_00a8_ori_l_dan;
        m68k_instruction [0x00b0 | reg] = m68k_00b0_ori_l_danxi;
        m68k_instruction [0x0200 | reg] = m68k_0200_andi_b_dn;
        m68k_instruction [0x0210 | reg] = m68k_0210_andi_b_an;
        m68k_instruction [0x0218 | reg] = m68k_0218_andi_b_anp;
        m68k_instruction [0x0220 | reg] = m68k_0220_andi_b_pan;
        m68k_instruction [0x0228 | reg] = m68k_0228_andi_b_dan;
        m68k_instruction [0x0230 | reg] = m68k_0230_andi_b_danxi;
        ...
        m68k_instruction [0x0a50 | reg] = m68k_0a50_eori_w_an;
        m68k_instruction [0x0a58 | reg] = m68k_0a58_eori_w_anp;
        m68k_instruction [0x0a60 | reg] = m68k_0a60_eori_w_pan;
        m68k_instruction [0x0a68 | reg] = m68k_0a68_eori_w_dan;
        m68k_instruction [0x0a70 | reg] = m68k_0a70_eori_w_danxi;
        m68k_instruction [0x0a80 | reg] = m68k_0a80_eori_l_dn;
        m68k_instruction [0x0a90 | reg] = m68k_0a90_eori_l_an;
        m68k_instruction [0x0a98 | reg] = m68k_0a98_eori_l_anp;
        m68k_instruction [0x0aa0 | reg] = m68k_0aa0_eori_l_pan;
        m68k_instruction [0x0aa8 | reg] = m68k_0aa8_eori_l_dan;
        m68k_instruction [0x0ab0 | reg] = m68k_0ab0_eori_l_danxi;
    ...
    m68k_instruction [0x0a78] = m68k_0a78_eori_w_aw;
    m68k_instruction [0x0a79] = m68k_0a79_eori_w_al;
    m68k_instruction [0x0a7c] = m68k_0a7c_eori_w_sr;
    m68k_instruction [0x0ab8] = m68k_0ab8_eori_l_aw;
    m68k_instruction [0x0ab9] = m68k_0ab9_eori_l_al;
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        m68k_instruction [0x4200 | reg] = m68k_4200_clr_b_dn;
        ...
        m68k_instruction [0x4240 | reg] = m68k_4240_clr_w_dn;
        m68k_instruction [0x4250 | reg] = m68k_4250_clr_w_an;
        m68k_instruction [0x4258 | reg] = m68k_4258_clr_w_anp;
        m68k_instruction [0x4260 | reg] = m68k_4260_clr_w_pan;
        m68k_instruction [0x4268 | reg] = m68k_4268_clr_w_dan;
        m68k_instruction [0x4270 | reg] = m68k_4270_clr_w_danxi;
    ...
    m68k_instruction [0x4279] = m68k_4279_clr_w_al;
    ...
    for (uint16_t data = 0; data < 8; data++)
    {
        for (uint16_t reg = 0; reg < 8; reg++)
        {
            ...
            m68k_instruction [0x5060 | (data << 9) | reg] = m68k_5060_addq_w_pan;
            m68k_instruction [0x5068 | (data << 9) | reg] = m68k_5068_addq_w_dan;
            m68k_instruction [0x5070 | (data << 9) | reg] = m68k_5070_addq_w_danxi;
            m68k_instruction [0x5080 | (data << 9) | reg] = m68k_5080_addq_l_dn;
            m68k_instruction [0x5088 | (data << 9) | reg] = m68k_5088_addq_l_an;
            m68k_instruction [0x5090 | (data << 9) | reg] = m68k_5090_addq_l_an;
            m68k_instruction [0x5098 | (data << 9) | reg] = m68k_5098_addq_l_anp;
            m68k_instruction [0x50a0 | (data << 9) | reg] = m68k_50a0_addq_l_pan;
            m68k_instruction [0x50a8 | (data << 9) | reg] = m68k_50a8_addq_l_dan;
            m68k_instruction [0x50b0 | (data << 9) | reg] = m68k_50b0_addq_l_danxi;
            m68k_instruction [0x5100 | (data << 9) | reg] = m68k_5100_subq_b_dn;
            m68k_instruction [0x5110 | (data << 9) | reg] = m68k_5110_subq_b_an;
            m68k_instruction [0x5118 | (data << 9) | reg] = m68k_5118_subq_b_anp;
            m68k_instruction [0x5120 | (data << 9) | reg] = m68k_5120_subq_b_pan;
            ...
            m68k_instruction [0x5160 | (data << 9) | reg] = m68k_5160_subq_w_pan;
            m68k_instruction [0x5168 | (data << 9) | reg] = m68k_5168_subq_w_dan;
            m68k_instruction [0x5170 | (data << 9) | reg] = m68k_5170_subq_w_danxi;
        ...
        m68k_instruction [0x5079 | (data << 9)] = m68k_5079_addq_w_al;
        m68k_instruction [0x50b8 | (data << 9)] = m68k_50b8_addq_l_aw;
        m68k_instruction [0x50b9 | (data << 9)] = m68k_50b9_addq_l_al;
        m68k_instruction [0x5138 | (data << 9)] = m68k_5138_subq_b_aw;
        m68k_instruction [0x5139 | (data << 9)] = m68k_5139_subq_b_al;
        m68k_instruction [0x5178 | (data << 9)] = m68k_5178_subq_w_aw;
        m68k_instruction [0x5179 | (data << 9)] = m68k_5179_subq_w_al;
    ...
    for (uint16_t dn = 0; dn < 8; dn++)
    {
        ...
        m68k_instruction [0x51c0 | dn] = m68k_51c0_sf_b_dn;
        m68k_instruction [0x51d0 | dn] = m68k_51d0_sf_b_an;
        m68k_instruction [0x51d8 | dn] = m68k_51d8_sf_b_anp;
        m68k_instruction [0x51e0 | dn] = m68k_51e0_sf_b_pan;
        m68k_instruction [0x51e8 | dn] = m68k_51e8_sf_b_dan;
        m68k_instruction [0x51f0 | dn] = m68k_51f0_sf_b_danxi;
        m68k_instruction [0x52c0 | dn] = m68k_52c0_shi_b_dn;
        m68k_instruction [0x52d0 | dn] = m68k_52d0_shi_b_an;
        m68k_instruction [0x52d8 | dn] = m68k_52d8_shi_b_anp;
        m68k_instruction [0x52e0 | dn] = m68k_52e0_shi_b_pan;
        m68k_instruction [0x52e8 | dn] = m68k_52e8_shi_b_dan;
        m68k_instruction [0x52f0 | dn] = m68k_52f0_shi_b_danxi;
        m68k_instruction [0x53c0 | dn] = m68k_53c0_sls_b_dn;
        m68k_instruction [0x53d0 | dn] = m68k_53d0_sls_b_an;
        m68k_instruction [0x53d8 | dn] = m68k_53d8_sls_b_anp;
        m68k_instruction [0x53e0 | dn] = m68k_53e0_sls_b_pan;
        m68k_instruction [0x53e8 | dn] = m68k_53e8_sls_b_dan;
        m68k_instruction [0x53f0 | dn] = m68k_53f0_sls_b_danxi;
        m68k_instruction [0x54c0 | dn] = m68k_54c0_scc_b_dn;
        m68k_instruction [0x54d0 | dn] = m68k_54d0_scc_b_an;
        m68k_instruction [0x54d8 | dn] = m68k_54d8_scc_b_anp;
        m68k_instruction [0x54e0 | dn] = m68k_54e0_scc_b_pan;
        m68k_instruction [0x54e8 | dn] = m68k_54e8_scc_b_dan;
        m68k_instruction [0x54f0 | dn] = m68k_54f0_scc_b_danxi;
        m68k_instruction [0x55c0 | dn] = m68k_55c0_scs_b_dn;
        m68k_instruction [0x55d0 | dn] = m68k_55d0_scs_b_an;
        m68k_instruction [0x55d8 | dn] = m68k_55d8_scs_b_anp;
        m68k_instruction [0x55e0 | dn] = m68k_55e0_scs_b_pan;
        m68k_instruction [0x55e8 | dn] = m68k_55e8_scs_b_dan;
        m68k_instruction [0x55f0 | dn] = m68k_55f0_scs_b_danxi;
        ...
        m68k_instruction [0x57c0 | dn] = m68k_57c0_seq_b_dn;
        m68k_instruction [0x57d0 | dn] = m68k_57d0_seq_b_an;
        m68k_instruction [0x57d8 | dn] = m68k_57d8_seq_b_anp;
        m68k_instruction [0x57e0 | dn] = m68k_57e0_seq_b_pan;
        m68k_instruction [0x57e8 | dn] = m68k_57e8_seq_b_dan;
        m68k_instruction [0x57f0 | dn] = m68k_57f0_seq_b_danxi;
        m68k_instruction [0x58c0 | dn] = m68k_58c0_svc_b_dn;
        m68k_instruction [0x58d0 | dn] = m68k_58d0_svc_b_an;
        m68k_instruction [0x58d8 | dn] = m68k_58d8_svc_b_anp;
        m68k_instruction [0x58e0 | dn] = m68k_58e0_svc_b_pan;
        m68k_instruction [0x58e8 | dn] = m68k_58e8_svc_b_dan;
        m68k_instruction [0x58f0 | dn] = m68k_58f0_svc_b_danxi;
        m68k_instruction [0x59c0 | dn] = m68k_59c0_svs_b_dn;
        m68k_instruction [0x59d0 | dn] = m68k_59d0_svs_b_an;
        m68k_instruction [0x59d8 | dn] = m68k_59d8_svs_b_anp;
        m68k_instruction [0x59e0 | dn] = m68k_59e0_svs_b_pan;
        m68k_instruction [0x59e8 | dn] = m68k_59e8_svs_b_dan;
        m68k_instruction [0x59f0 | dn] = m68k_59f0_svs_b_danxi;
        m68k_instruction [0x5ac0 | dn] = m68k_5ac0_spl_b_dn;
        m68k_instruction [0x5ad0 | dn] = m68k_5ad0_spl_b_an;
        m68k_instruction [0x5ad8 | dn] = m68k_5ad8_spl_b_anp;
        m68k_instruction [0x5ae0 | dn] = m68k_5ae0_spl_b_pan;
        m68k_instruction [0x5ae8 | dn] = m68k_5ae8_spl_b_dan;
        m68k_instruction [0x5af0 | dn] = m68k_5af0_spl_b_danxi;
        m68k_instruction [0x5bc0 | dn] = m68k_5bc0_smi_b_dn;
        m68k_instruction [0x5bd0 | dn] = m68k_5bd0_smi_b_an;
        m68k_instruction [0x5bd8 | dn] = m68k_5bd8_smi_b_anp;
        m68k_instruction [0x5be0 | dn] = m68k_5be0_smi_b_pan;
        m68k_instruction [0x5be8 | dn] = m68k_5be8_smi_b_dan;
        m68k_instruction [0x5bf0 | dn] = m68k_5bf0_smi_b_danxi;
        m68k_instruction [0x5cc0 | dn] = m68k_5cc0_sge_b_dn;
        m68k_instruction [0x5cd0 | dn] = m68k_5cd0_sge_b_an;
        m68k_instruction [0x5cd8 | dn] = m68k_5cd8_sge_b_anp;
        m68k_instruction [0x5ce0 | dn] = m68k_5ce0_sge_b_pan;
        m68k_instruction [0x5ce8 | dn] = m68k_5ce8_sge_b_dan;
        m68k_instruction [0x5cf0 | dn] = m68k_5cf0_sge_b_danxi;
        m68k_instruction [0x5dc0 | dn] = m68k_5dc0_slt_b_dn;
        m68k_instruction [0x5dd0 | dn] = m68k_5dd0_slt_b_an;
        m68k_instruction [0x5dd8 | dn] = m68k_5dd8_slt_b_anp;
        m68k_instruction [0x5de0 | dn] = m68k_5de0_slt_b_pan;
        m68k_instruction [0x5de8 | dn] = m68k_5de8_slt_b_dan;
        m68k_instruction [0x5df0 | dn] = m68k_5df0_slt_b_danxi;
        m68k_instruction [0x5ec0 | dn] = m68k_5ec0_sgt_b_dn;
        m68k_instruction [0x5ed0 | dn] = m68k_5ed0_sgt_b_an;
        m68k_instruction [0x5ed8 | dn] = m68k_5ed8_sgt_b_anp;
        m68k_instruction [0x5ee0 | dn] = m68k_5ee0_sgt_b_pan;
        m68k_instruction [0x5ee8 | dn] = m68k_5ee8_sgt_b_dan;
        m68k_instruction [0x5ef0 | dn] = m68k_5ef0_sgt_b_danxi;
        m68k_instruction [0x5fc0 | dn] = m68k_5fc0_sle_b_dn;
        m68k_instruction [0x5fd0 | dn] = m68k_5fd0_sle_b_an;
        m68k_instruction [0x5fd8 | dn] = m68k_5fd8_sle_b_anp;
        m68k_instruction [0x5fe0 | dn] = m68k_5fe0_sle_b_pan;
        m68k_instruction [0x5fe8 | dn] = m68k_5fe8_sle_b_dan;
        m68k_instruction [0x5ff0 | dn] = m68k_5ff0_sle_b_danxi;
    }
    m68k_instruction [0x50f8] = m68k_50f8_st_b_aw;
    m68k_instruction [0x50f9] = m68k_50f9_st_b_al;
    m68k_instruction [0x51f8] = m68k_51f8_sf_b_aw;
    m68k_instruction [0x51f9] = m68k_51f9_sf_b_al;
    m68k_instruction [0x52f8] = m68k_52f8_shi_b_aw;
    m68k_instruction [0x52f9] = m68k_52f9_shi_b_al;
    m68k_instruction [0x53f8] = m68k_53f8_sls_b_aw;
    m68k_instruction [0x53f9] = m68k_53f9_sls_b_al;
    m68k_instruction [0x54f8] = m68k_54f8_scc_b_aw;
    m68k_instruction [0x54f9] = m68k_54f9_scc_b_al;
    m68k_instruction [0x55f8] = m68k_55f8_scs_b_aw;
    m68k_instruction [0x55f9] = m68k_55f9_scs_b_al;
    m68k_instruction [0x56f8] = m68k_56f8_sne_b_aw;
    m68k_instruction [0x56f9] = m68k_56f9_sne_b_al;
    m68k_instruction [0x57f8] = m68k_57f8_seq_b_aw;
    m68k_instruction [0x57f9] = m68k_57f9_seq_b_al;
    m68k_instruction [0x58f8] = m68k_58f8_svc_b_aw;
    m68k_instruction [0x58f9] = m68k_58f9_svc_b_al;
    m68k_instruction [0x59f8] = m68k_59f8_svs_b_aw;
    m68k_instruction [0x59f9] = m68k_59f9_svs_b_al;
    m68k_instruction [0x5af8] = m68k_5af8_spl_b_aw;
    m68k_instruction [0x5af9] = m68k_5af9_spl_b_al;
    m68k_instruction [0x5bf8] = m68k_5bf8_smi_b_aw;
    m68k_instruction [0x5bf9] = m68k_5bf9_smi_b_al;
    m68k_instruction [0x5cf8] = m68k_5cf8_sge_b_aw;
    m68k_instruction [0x5cf9] = m68k_5cf9_sge_b_al;
    m68k_instruction [0x5df8] = m68k_5df8_slt_b_aw;
    m68k_instruction [0x5df9] = m68k_5df9_slt_b_al;
    m68k_instruction [0x5ef8] = m68k_5ef8_sgt_b_aw;
    m68k_instruction [0x5ef9] = m68k_5ef9_sgt_b_al;
    m68k_instruction [0x5ff8] = m68k_5ff8_sle_b_aw;
    m68k_instruction [0x5ff9] = m68k_5ff9_sle_b_al;
    ...
    for (uint16_t reg_a = 0; reg_a < 8; reg_a++)
    {
        for (uint16_t reg_b = 0; reg_b < 8; reg_b++)
        {
            ...
            m68k_instruction [0x8010 | (reg_a << 9) | reg_b] = m68k_8010_or_b_dn_an;
            m68k_instruction [0x8018 | (reg_a << 9) | reg_b] = m68k_8018_or_b_dn_anp;
            m68k_instruction [0x8020 | (reg_a << 9) | reg_b] = m68k_8020_or_b_dn_pan;
            m68k_instruction [0x8028 | (reg_a << 9) | reg_b] = m68k_8028_or_b_dn_dan;
            m68k_instruction [0x8030 | (reg_a << 9) | reg_b] = m68k_8030_or_b_dn_danxi;
            m68k_instruction [0x8040 | (reg_a << 9) | reg_b] = m68k_8040_or_w_dn_dn;
            m68k_instruction [0x8050 | (reg_a << 9) | reg_b] = m68k_8050_or_w_dn_an;
            m68k_instruction [0x8058 | (reg_a << 9) | reg_b] = m68k_8058_or_w_dn_anp;
            m68k_instruction [0x8060 | (reg_a << 9) | reg_b] = m68k_8060_or_w_dn_pan;
            m68k_instruction [0x8068 | (reg_a << 9) | reg_b] = m68k_8068_or_w_dn_dan;
            m68k_instruction [0x8070 | (reg_a << 9) | reg_b] = m68k_8070_or_w_dn_danxi;
            m68k_instruction [0x8080 | (reg_a << 9) | reg_b] = m68k_8080_or_l_dn_dn;
            m68k_instruction [0x8090 | (reg_a << 9) | reg_b] = m68k_8090_or_l_dn_an;
            m68k_instruction [0x8098 | (reg_a << 9) | reg_b] = m68k_8098_or_l_dn_anp;
            m68k_instruction [0x80a0 | (reg_a << 9) | reg_b] = m68k_80a0_or_l_dn_pan;
            m68k_instruction [0x80a8 | (reg_a << 9) | reg_b] = m68k_80a8_or_l_dn_dan;
            m68k_instruction [0x80b0 | (reg_a << 9) | reg_b] = m68k_80b0_or_l_dn_danxi;
            m68k_instruction [0x8110 | (reg_a << 9) | reg_b] = m68k_8110_or_b_an_dn;
            m68k_instruction [0x8118 | (reg_a << 9) | reg_b] = m68k_8118_or_b_anp_dn;
            m68k_instruction [0x8120 | (reg_a << 9) | reg_b] = m68k_8120_or_b_pan_dn;
            m68k_instruction [0x8128 | (reg_a << 9) | reg_b] = m68k_8128_or_b_dan_dn;
            m68k_instruction [0x8130 | (reg_a << 9) | reg_b] = m68k_8130_or_b_danxi_dn;
            m68k_instruction [0x8150 | (reg_a << 9) | reg_b] = m68k_8150_or_w_an_dn;
            m68k_instruction [0x8158 | (reg_a << 9) | reg_b] = m68k_8158_or_w_anp_dn;
            m68k_instruction [0x8160 | (reg_a << 9) | reg_b] = m68k_8160_or_w_pan_dn;
            m68k_instruction [0x8168 | (reg_a << 9) | reg_b] = m68k_8168_or_w_dan_dn;
            m68k_instruction [0x8170 | (reg_a << 9) | reg_b] = m68k_8170_or_w_danxi_dn;
            m68k_instruction [0x8190 | (reg_a << 9) | reg_b] = m68k_8190_or_l_an_dn;
            m68k_instruction [0x8198 | (reg_a << 9) | reg_b] = m68k_8198_or_l_anp_dn;
            m68k_instruction [0x81a0 | (reg_a << 9) | reg_b] = m68k_81a0_or_l_pan_dn;
            m68k_instruction [0x81a8 | (reg_a << 9) | reg_b] = m68k_81a8_or_l_dan_dn;
            m68k_instruction [0x81b0 | (reg_a << 9) | reg_b] = m68k_81b0_or_l_danxi_dn;
        }
        m68k_instruction [0x8038 | (reg_a << 9)] = m68k_8038_or_b_dn_aw;
        m68k_instruction [0x8039 | (reg_a << 9)] = m68k_8039_or_b_dn_al;
        m68k_instruction [0x803a | (reg_a << 9)] = m68k_803a_or_b_dn_dpc;
        m68k_instruction [0x803b | (reg_a << 9)] = m68k_803b_or_b_dn_dpcxi;
        m68k_instruction [0x803c | (reg_a << 9)] = m68k_803c_or_b_dn_imm;
        m68k_instruction [0x8078 | (reg_a << 9)] = m68k_8078_or_w_dn_aw;
        m68k_instruction [0x8079 | (reg_a << 9)] = m68k_8079_or_w_dn_al;
        m68k_instruction [0x807a | (reg_a << 9)] = m68k_807a_or_w_dn_dpc;
        m68k_instruction [0x807b | (reg_a << 9)] = m68k_807b_or_w_dn_dpcxi;
        m68k_instruction [0x807c | (reg_a << 9)] = m68k_807c_or_w_dn_imm;
        m68k_instruction [0x80b8 | (reg_a << 9)] = m68k_80b8_or_l_dn_aw;
        m68k_instruction [0x80b9 | (reg_a << 9)] = m68k_80b9_or_l_dn_al;
        m68k_instruction [0x80ba | (reg_a << 9)] = m68k_80ba_or_l_dn_dpc;
        m68k_instruction [0x80bb | (reg_a << 9)] = m68k_80bb_or_l_dn_dpcxi;
        m68k_instruction [0x80bc | (reg_a << 9)] = m68k_80bc_or_l_dn_imm;
        m68k_instruction [0x8138 | (reg_a << 9)] = m68k_8138_or_b_aw_dn;
        m68k_instruction [0x8139 | (reg_a << 9)] = m68k_8139_or_b_al_dn;
        m68k_instruction [0x8178 | (reg_a << 9)] = m68k_8178_or_w_aw_dn;
        m68k_instruction [0x8179 | (reg_a << 9)] = m68k_8179_or_w_al_dn;
        m68k_instruction [0x81b8 | (reg_a << 9)] = m68k_81b8_or_l_aw_dn;
        m68k_instruction [0x81b9 | (reg_a << 9)] = m68k_81b9_or_l_al_dn;
    }
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        for (uint16_t ea = 0; ea < 8; ea++)
        {
            ...
            m68k_instruction [0xb058 | (reg << 9) | ea] = m68k_b058_cmp_w_dn_anp;
            m68k_instruction [0xb060 | (reg << 9) | ea] = m68k_b060_cmp_w_dn_pan;
            m68k_instruction [0xb068 | (reg << 9) | ea] = m68k_b068_cmp_w_dn_dan;
            m68k_instruction [0xb070 | (reg << 9) | ea] = m68k_b070_cmp_w_dn_danxi;
            m68k_instruction [0xb080 | (reg << 9) | ea] = m68k_b080_cmp_l_dn_dn;
            m68k_instruction [0xb088 | (reg << 9) | ea] = m68k_b088_cmp_l_dn_an;
            m68k_instruction [0xb090 | (reg << 9) | ea] = m68k_b090_cmp_l_dn_an;
            m68k_instruction [0xb098 | (reg << 9) | ea] = m68k_b098_cmp_l_dn_anp;
            m68k_instruction [0xb0a0 | (reg << 9) | ea] = m68k_b0a0_cmp_l_dn_pan;
            m68k_instruction [0xb0a8 | (reg << 9) | ea] = m68k_b0a8_cmp_l_dn_dan;
            m68k_instruction [0xb0b0 | (reg << 9) | ea] = m68k_b0b0_cmp_l_dn_danxi;
        ...
        m68k_instruction [0xb079 | (reg << 9)] = m68k_b079_cmp_w_dn_al;
        m68k_instruction [0xb07a | (reg << 9)] = m68k_b07a_cmp_w_dn_dpc;
        m68k_instruction [0xb07b | (reg << 9)] = m68k_b07b_cmp_w_dn_dpcxi;
        m68k_instruction [0xb07c | (reg << 9)] = m68k_b07c_cmp_w_dn_imm;
        m68k_instruction [0xb0b8 | (reg << 9)] = m68k_b0b8_cmp_l_dn_aw;
        m68k_instruction [0xb0b9 | (reg << 9)] = m68k_b0b9_cmp_l_dn_al;
        m68k_instruction [0xb0ba | (reg << 9)] = m68k_b0ba_cmp_l_dn_dpc;
        m68k_instruction [0xb0bb | (reg << 9)] = m68k_b0bb_cmp_l_dn_dpcxi;
        m68k_instruction [0xb0bc | (reg << 9)] = m68k_b0bc_cmp_l_dn_imm;
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        for (uint16_t ea = 0; ea < 8; ea++)
        {
            m68k_instruction [0xb100 | (reg << 9) | ea] = m68k_b100_eor_b_dn_dn;
            m68k_instruction [0xb110 | (reg << 9) | ea] = m68k_b110_eor_b_an_dn;
            m68k_instruction [0xb118 | (reg << 9) | ea] = m68k_b118_eor_b_anp_dn;
            m68k_instruction [0xb120 | (reg << 9) | ea] = m68k_b120_eor_b_pan_dn;
            m68k_instruction [0xb128 | (reg << 9) | ea] = m68k_b128_eor_b_dan_dn;
            m68k_instruction [0xb130 | (reg << 9) | ea] = m68k_b130_eor_b_danxi_dn;
            m68k_instruction [0xb140 | (reg << 9) | ea] = m68k_b140_eor_w_dn_dn;
            m68k_instruction [0xb150 | (reg << 9) | ea] = m68k_b150_eor_w_an_dn;
            m68k_instruction [0xb158 | (reg << 9) | ea] = m68k_b158_eor_w_anp_dn;
            m68k_instruction [0xb160 | (reg << 9) | ea] = m68k_b160_eor_w_pan_dn;
            m68k_instruction [0xb168 | (reg << 9) | ea] = m68k_b168_eor_w_dan_dn;
            m68k_instruction [0xb170 | (reg << 9) | ea] = m68k_b170_eor_w_danxi_dn;
            m68k_instruction [0xb180 | (reg << 9) | ea] = m68k_b180_eor_l_dn_dn;
            m68k_instruction [0xb190 | (reg << 9) | ea] = m68k_b190_eor_l_an_dn;
            m68k_instruction [0xb198 | (reg << 9) | ea] = m68k_b198_eor_l_anp_dn;
            m68k_instruction [0xb1a0 | (reg << 9) | ea] = m68k_b1a0_eor_l_pan_dn;
            m68k_instruction [0xb1a8 | (reg << 9) | ea] = m68k_b1a8_eor_l_dan_dn;
            m68k_instruction [0xb1b0 | (reg << 9) | ea] = m68k_b1b0_eor_l_danxi_dn;
        }
        m68k_instruction [0xb138 | (reg << 9)] = m68k_b138_eor_b_aw_dn;
        m68k_instruction [0xb139 | (reg << 9)] = m68k_b139_eor_b_al_dn;
        m68k_instruction [0xb178 | (reg << 9)] = m68k_b178_eor_w_aw_dn;
        m68k_instruction [0xb179 | (reg << 9)] = m68k_b179_eor_w_al_dn;
        m68k_instruction [0xb1b8 | (reg << 9)] = m68k_b1b8_eor_l_aw_dn;
        m68k_instruction [0xb1b9 | (reg << 9)] = m68k_b1b9_eor_l_al_dn;
    }
    ...
    for (uint16_t reg_a = 0; reg_a < 8; reg_a++)
    {
        for (uint16_t reg_b = 0; reg_b < 8; reg_b++)
        {
            ...
            m68k_instruction [0xc050 | (reg_a << 9) | reg_b] = m68k_c050_and_w_dn_an;
            m68k_instruction [0xc058 | (reg_a << 9) | reg_b] = m68k_c058_and_w_dn_anp;
            m68k_instruction [0xc060 | (reg_a << 9) | reg_b] = m68k_c060_and_w_dn_pan;
            m68k_instruction [0xc068 | (reg_a << 9) | reg_b] = m68k_c068_and_w_dn_dan;
            m68k_instruction [0xc070 | (reg_a << 9) | reg_b] = m68k_c070_and_w_dn_danxi;
            m68k_instruction [0xc080 | (reg_a << 9) | reg_b] = m68k_c080_and_l_dn_dn;
            m68k_instruction [0xc090 | (reg_a << 9) | reg_b] = m68k_c090_and_l_dn_an;
            m68k_instruction [0xc098 | (reg_a << 9) | reg_b] = m68k_c098_and_l_dn_anp;
            m68k_instruction [0xc0a0 | (reg_a << 9) | reg_b] = m68k_c0a0_and_l_dn_pan;
            m68k_instruction [0xc0a8 | (reg_a << 9) | reg_b] = m68k_c0a8_and_l_dn_dan;
            m68k_instruction [0xc0b0 | (reg_a << 9) | reg_b] = m68k_c0b0_and_l_dn_danxi;
            ...
            m68k_instruction [0xc150 | (reg_a << 9) | reg_b] = m68k_c150_and_w_an_dn;
            m68k_instruction [0xc158 | (reg_a << 9) | reg_b] = m68k_c158_and_w_anp_dn;
            m68k_instruction [0xc160 | (reg_a << 9) | reg_b] = m68k_c160_and_w_pan_dn;
            m68k_instruction [0xc168 | (reg_a << 9) | reg_b] = m68k_c168_and_w_dan_dn;
            m68k_instruction [0xc170 | (reg_a << 9) | reg_b] = m68k_c170_and_w_danxi_dn;
            m68k_instruction [0xc190 | (reg_a << 9) | reg_b] = m68k_c190_and_l_an_dn;
            m68k_instruction [0xc198 | (reg_a << 9) | reg_b] = m68k_c198_and_l_anp_dn;
            m68k_instruction [0xc1a0 | (reg_a << 9) | reg_b] = m68k_c1a0_and_l_pan_dn;
            m68k_instruction [0xc1a8 | (reg_a << 9) | reg_b] = m68k_c1a8_and_l_dan_dn;
            m68k_instruction [0xc1b0 | (reg_a << 9) | reg_b] = m68k_c1b0_and_l_danxi_dn;
        }
        ...
        m68k_instruction [0xc078 | (reg_a << 9)] = m68k_c078_and_w_dn_aw;
        m68k_instruction [0xc079 | (reg_a << 9)] = m68k_c079_and_w_dn_al;
        m68k_instruction [0xc07a | (reg_a << 9)] = m68k_c07a_and_w_dn_dpc;
        m68k_instruction [0xc07b | (reg_a << 9)] = m68k_c07b_and_w_dn_dpcxi;
        m68k_instruction [0xc07c | (reg_a << 9)] = m68k_c07c_and_w_dn_imm;
        m68k_instruction [0xc0b8 | (reg_a << 9)] = m68k_c0b8_and_l_dn_aw;
        m68k_instruction [0xc0b9 | (reg_a << 9)] = m68k_c0b9_and_l_dn_al;
        m68k_instruction [0xc0ba | (reg_a << 9)] = m68k_c0ba_and_l_dn_dpc;
        m68k_instruction [0xc0bb | (reg_a << 9)] = m68k_c0bb_and_l_dn_dpcxi;
        m68k_instruction [0xc0bc | (reg_a << 9)] = m68k_c0bc_and_l_dn_imm;
        m68k_instruction [0xc138 | (reg_a << 9)] = m68k_c138_and_b_aw_dn;
        m68k_instruction [0xc139 | (reg_a << 9)] = m68k_c139_and_b_al_dn;
        m68k_instruction [0xc178 | (reg_a << 9)] = m68k_c178_and_w_aw_dn;
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        for (uint16_t ea = 0; ea < 8; ea++)
        {
            ...
            m68k_instruction [0xd090 | (reg << 9) | ea] = m68k_d090_add_l_dn_an;
            m68k_instruction [0xd098 | (reg << 9) | ea] = m68k_d098_add_l_dn_anp;
            m68k_instruction [0xd0a0 | (reg << 9) | ea] = m68k_d0a0_add_l_dn_pan;
            m68k_instruction [0xd0a8 | (reg << 9) | ea] = m68k_d0a8_add_l_dn_dan;
            m68k_instruction [0xd0b0 | (reg << 9) | ea] = m68k_d0b0_add_l_dn_danxi;
            ...
            m68k_instruction [0xd0d8 | (reg << 9) | ea] = m68k_d0d8_adda_w_an_anp;
            ...
            m68k_instruction [0xd100 | (reg << 9) | ea] = m68k_d100_addx_b_dn_dn;
            m68k_instruction [0xd108 | (reg << 9) | ea] = m68k_d108_addx_b_pan_pan;
            ...
            m68k_instruction [0xd140 | (reg << 9) | ea] = m68k_d140_addx_w_dn_dn;
            m68k_instruction [0xd148 | (reg << 9) | ea] = m68k_d148_addx_w_pan_pan;
            ...
            m68k_instruction [0xd180 | (reg << 9) | ea] = m68k_d180_addx_l_dn_dn;
            m68k_instruction [0xd188 | (reg << 9) | ea] = m68k_d188_addx_l_pan_pan;
            m68k_instruction [0xd190 | (reg << 9) | ea] = m68k_d190_add_l_an_dn;
            m68k_instruction [0xd198 | (reg << 9) | ea] = m68k_d198_add_l_anp_dn;
            m68k_instruction [0xd1a0 | (reg << 9) | ea] = m68k_d1a0_add_l_pan_dn;
            m68k_instruction [0xd1a8 | (reg << 9) | ea] = m68k_d1a8_add_l_dan_dn;
            m68k_instruction [0xd1b0 | (reg << 9) | ea] = m68k_d1b0_add_l_danxi_dn;
            m68k_instruction [0xd1c0 | (reg << 9) | ea] = m68k_d1c0_adda_l_an_dn;
            m68k_instruction [0xd1c8 | (reg << 9) | ea] = m68k_d1c8_adda_l_an_an;
            m68k_instruction [0xd1d0 | (reg << 9) | ea] = m68k_d1d0_adda_l_an_an;
            m68k_instruction [0xd1d8 | (reg << 9) | ea] = m68k_d1d8_adda_l_an_anp;
            m68k_instruction [0xd1e0 | (reg << 9) | ea] = m68k_d1e0_adda_l_an_pan;
            m68k_instruction [0xd1e8 | (reg << 9) | ea] = m68k_d1e8_adda_l_an_dan;
        ...
        m68k_instruction [0xd0bb | (reg << 9)] = m68k_d0bb_add_l_dn_dpcxi;
        m68k_instruction [0xd0bc | (reg << 9)] = m68k_d0bc_add_l_dn_imm;
        m68k_instruction [0xd0f8 | (reg << 9)] = m68k_d0f8_adda_w_an_aw;
        m68k_instruction [0xd0f9 | (reg << 9)] = m68k_d0f9_adda_w_an_al;
        ...
        m68k_instruction [0xd1b9 | (reg << 9)] = m68k_d1b9_add_l_al_dn;
        m68k_instruction [0xd1f8 | (reg << 9)] = m68k_d1f8_adda_l_an_aw;
        m68k_instruction [0xd1f9 | (reg << 9)] = m68k_d1f9_adda_l_an_al;
        m68k_instruction [0xd1fa | (reg << 9)] = m68k_d1fa_adda_l_an_dpc;
        m68k_instruction [0xd1fb | (reg << 9)] = m68k_d1fb_adda_l_an_dpcxi;
        m68k_instruction [0xd1fc | (reg << 9)] = m68k_d1fc_adda_l_an_imm;
    }
    ...
    for (uint16_t reg = 0; reg < 8; reg++)
    {
        ...
        {
            m68k_instruction [0xe000 | (count << 9) | reg] = m68k_e000_asr_b_dn_imm;
            m68k_instruction [0xe008 | (count << 9) | reg] = m68k_e008_lsr_b_dn_imm;
            m68k_instruction [0xe010 | (count << 9) | reg] = m68k_e010_roxr_b_dn_imm;
            m68k_instruction [0xe018 | (count << 9) | reg] = m68k_e018_ror_b_dn_imm;
            m68k_instruction [0xe020 | (count << 9) | reg] = m68k_e020_asr_b_dn_dn;
            m68k_instruction [0xe028 | (count << 9) | reg] = m68k_e028_lsr_b_dn_dn;
            m68k_instruction [0xe030 | (count << 9) | reg] = m68k_e030_roxr_b_dn_dn;
            ...
            m68k_instruction [0xe050 | (count << 9) | reg] = m68k_e050_roxr_w_dn_imm;
            m68k_instruction [0xe058 | (count << 9) | reg] = m68k_e058_ror_w_dn_imm;
            m68k_instruction [0xe060 | (count << 9) | reg] = m68k_e060_asr_w_dn_dn;
            m68k_instruction [0xe068 | (count << 9) | reg] = m68k_e068_lsr_w_dn_dn;
            m68k_instruction [0xe070 | (count << 9) | reg] = m68k_e070_roxr_w_dn_dn;
            m68k_instruction [0xe078 | (count << 9) | reg] = m68k_e078_ror_w_dn_dn;
            m68k_instruction [0xe080 | (count << 9) | reg] = m68k_e080_asr_l_dn_imm;
            m68k_instruction [0xe088 | (count << 9) | reg] = m68k_e088_lsr_l_dn_imm;
            m68k_instruction [0xe090 | (count << 9) | reg] = m68k_e090_roxr_l_dn_imm;
            m68k_instruction [0xe098 | (count << 9) | reg] = m68k_e098_ror_l_dn_imm;
            m68k_instruction [0xe0a0 | (count << 9) | reg] = m68k_e0a0_asr_l_dn_dn;
            m68k_instruction [0xe0a8 | (count << 9) | reg] = m68k_e0a8_lsr_l_dn_dn;
            m68k_instruction [0xe0b0 | (count << 9) | reg] = m68k_e0b0_roxr_l_dn_dn;
            m68k_instruction [0xe0b8 | (count << 9) | reg] = m68k_e0b8_ror_l_dn_dn;
            m68k_instruction [0xe100 | (count << 9) | reg] = m68k_e100_asl_b_dn_imm;
            m68k_instruction [0xe108 | (count << 9) | reg] = m68k_e108_lsl_b_dn_imm;
            m68k_instruction [0xe110 | (count << 9) | reg] = m68k_e110_roxl_b_dn_imm;
            m68k_instruction [0xe118 | (count << 9) | reg] = m68k_e118_rol_b_dn_imm;
            m68k_instruction [0xe120 | (count << 9) | reg] = m68k_e120_asl_b_dn_dn;
            m68k_instruction [0xe128 | (count << 9) | reg] = m68k_e128_lsl_b_dn_dn;
            m68k_instruction [0xe130 | (count << 9) | reg] = m68k_e130_roxl_b_dn_dn;
            ...
            m68k_instruction [0xe170 | (count << 9) | reg] = m68k_e170_roxl_w_dn_dn;
            ...
            m68k_instruction [0xe190 | (count << 9) | reg] = m68k_e190_roxl_l_dn_imm;
            m68k_instruction [0xe198 | (count << 9) | reg] = m68k_e198_rol_l_dn_imm;
            m68k_instruction [0xe1a0 | (count << 9) | reg] = m68k_e1a0_asl_l_dn_dn;
            m68k_instruction [0xe1a8 | (count << 9) | reg] = m68k_e1a8_lsl_l_dn_dn;
            m68k_instruction [0xe1b0 | (count << 9) | reg] = m68k_e1b0_roxl_l_dn_dn;
            m68k_instruction [0xe1b8 | (count << 9) | reg] = m68k_e1b8_rol_l_dn_dn;
        }
        m68k_instruction [0xe0d0 | reg] = m68k_e0d0_asr_w_an;
        m68k_instruction [0xe0d8 | reg] = m68k_e0d8_asr_w_anp;
        m68k_instruction [0xe0e0 | reg] = m68k_e0e0_asr_w_pan;
        m68k_instruction [0xe0e8 | reg] = m68k_e0e8_asr_w_dan;
        m68k_instruction [0xe0f0 | reg] = m68k_e0f0_asr_w_danxi;
        ...
        m68k_instruction [0xe2d0 | reg] = m68k_e2d0_lsr_w_an;
        m68k_instruction [0xe2d8 | reg] = m68k_e2d8_lsr_w_anp;
        m68k_instruction [0xe2e0 | reg] = m68k_e2e0_lsr_w_pan;
        m68k_instruction [0xe2e8 | reg] = m68k_e2e8_lsr_w_dan;
        m68k_instruction [0xe2f0 | reg] = m68k_e2f0_lsr_w_danxi;
        m68k_instruction [0xe3d0 | reg] = m68k_e3d0_lsl_w_an;
        m68k_instruction [0xe3d8 | reg] = m68k_e3d8_lsl_w_anp;
        m68k_instruction [0xe3e0 | reg] = m68k_e3e0_lsl_w_pan;
        m68k_instruction [0xe3e8 | reg] = m68k_e3e8_lsl_w_dan;
        m68k_instruction [0xe3f0 | reg] = m68k_e3f0_lsl_w_danxi;
        m68k_instruction [0xe4d0 | reg] = m68k_e4d0_roxr_w_an;
        m68k_instruction [0xe4d8 | reg] = m68k_e4d8_roxr_w_anp;
        m68k_instruction [0xe4e0 | reg] = m68k_e4e0_roxr_w_pan;
        m68k_instruction [0xe4e8 | reg] = m68k_e4e8_roxr_w_dan;
        m68k_instruction [0xe4f0 | reg] = m68k_e4f0_roxr_w_danxi;
        m68k_instruction [0xe5d0 | reg] = m68k_e5d0_roxl_w_an;
        m68k_instruction [0xe5d8 | reg] = m68k_e5d8_roxl_w_anp;
        m68k_instruction [0xe5e0 | reg] = m68k_e5e0_roxl_w_pan;
        m68k_instruction [0xe5e8 | reg] = m68k_e5e8_roxl_w_dan;
        m68k_instruction [0xe5f0 | reg] = m68k_e5f0_roxl_w_danxi;
    ...
    m68k_instruction [0xe0f8] = m68k_e0f8_asr_w_aw;
    m68k_instruction [0xe0f9] = m68k_e0f9_asr_w_al;
    m68k_instruction [0xe1f8] = m68k_e1f8_asl_w_aw;
    m68k_instruction [0xe1f9] = m68k_e1f9_asl_w_al;
    m68k_instruction [0xe2f8] = m68k_e2f8_lsr_w_aw;
    m68k_instruction [0xe2f9] = m68k_e2f9_lsr_w_al;
    m68k_instruction [0xe3f8] = m68k_e3f8_lsl_w_aw;
    m68k_instruction [0xe3f9] = m68k_e3f9_lsl_w_al;
    m68k_instruction [0xe4f8] = m68k_e4f8_roxr_w_aw;
    m68k_instruction [0xe4f9] = m68k_e4f9_roxr_w_al;
    m68k_instruction [0xe5f8] = m68k_e5f8_roxl_w_aw;
    m68k_instruction [0xe5f9] = m68k_e5f9_roxl_w_al;
